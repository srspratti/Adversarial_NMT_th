...
class Discriminator_bert(nn.Module):
    def __init__(self, args, src_dict, dst_dict, use_cuda = True):
        ...
        bert_model_name_src = "bert-base-uncased"  # Change if needed
        bert_model_name_trg = "bert-base-uncased"  # Change if needed
        ...
        self.conv1 = nn.Sequential(
            Conv2d(in_channels=2*bert_embedding_dim,
                   out_channels=512,
                   kernel_size=3,
                   stride=1,
                   padding=1),
    ...
    def forward(self, src_sentence, trg_sentence):
        ...
        # Generate BERT embeddings for both source and target sentences
        with torch.no_grad():
            src_out = self.bert_src(src_sentence)[0]
            trg_out = self.bert_trg(trg_sentence)[0]
        ...
        # Not sure if we need to include this or not, for now commenting it, change it later if necessary
